(function () {
  ...
  hint.textContent = isCoarse
    ? "Tap to slice the cheese"
    : "Scroll to slice the cheese";
  app.appendChild(hint);

  const CONFIG = {
    sliceThickness: 14,
    // Keep lock at or above bounce duration (.55s) to avoid overlapping bounces.
    animationGap: 300,
  };

  const maxHeight = parseInt(
    getComputedStyle(cheese).getPropertyValue("--cheese-max-height"),
    10,
  );
  let cutAmount = 0;
  let locked = false;
  ...
  function sliceCheese() {
    ...
    // Restart animation on next frame without forcing synchronous layout
    requestAnimationFrame(() => slicer.classList.add("slice-move"));
    ...
    cheese
      .querySelector(".cheese-fill")
      .style.setProperty("--cheese-cut", cutAmount + "px");
    updateSlicerOffset();
  ...
  function restart() {
    ...
    cheese
      .querySelector(".cheese-fill")
      .style.setProperty("--cheese-cut", "0px");
  ...
  slicer.addEventListener("animationend", (e) => {
    if (e.animationName === "slicerBounce")
      slicer.classList.remove("slice-move");
  });
